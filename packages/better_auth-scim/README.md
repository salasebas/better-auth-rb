...
Options use Ruby snake_case names: `store_scim_token`, `default_scim`,
`provider_ownership`, `required_role`, `link_existing_users`,
`can_generate_token`, `before_scim_token_generated`, and
`after_scim_token_generated`.

Existing-user linking is fail closed. A matching email returns SCIM `409`
unless `link_existing_users: true` is explicitly configured or every constraint
in a policy hash succeeds:
...
BetterAuth::Plugins.scim(
  link_existing_users: {
    ...
    should_link_user: ->(payload) { payload[:user]["email"] == payload[:email] }
  },
  can_generate_token: ->(payload) { !payload[:member].nil? }
)
```

`can_generate_token` runs after built-in ownership, membership, and role checks,
so it can only further restrict token issuance. Migrate legacy installations
deliberately: use `link_existing_users: true` only when every token issuer is
trusted to attach existing identities; otherwise choose positive policy
constraints.
...
Provider IDs cannot collide with the six fixed account provider IDs (`credential`,
`email-otp`, `magic-link`, `phone-number`, `anonymous`, and `siwe`), configured
social providers (including disabled entries), resolved providers, or SSO providers.

Email changes are lowercased and uniqueness checked. A real email change resets
`emailVerified`; a case-only write preserves it. Writing `active: false` requires
the admin plugin, maps to its enforced banned state, and revokes all sessions.

For an organization-scoped token, deleting a SCIM user removes only that
organization membership, its organization team memberships, and the current
SCIM account; the global user remains. For a personal token, deletion unlinks
the current SCIM account and deletes the global user only if it was the sole
identity. Deleting a provider connection only invalidates its token and retains
