  ...
  module Plugins
    ...
    def oauth_revoke_endpoint(config)
      Endpoint.new(path: "/oauth2/revoke", method: "POST", metadata: oauth_openapi_for(:revoke).merge(allowed_media_types: ["application/x-www-form-urlencoded", "application/json"])) do |ctx|
        ...
        if body["token_type_hint"].to_s == "access_token" && OAuthProtocol.find_token_by_hint(config[:store], body["token"].to_s, "refresh_token", **token_options.slice(:prefix, :format_refresh_token))
          raise APIError.new("BAD_REQUEST", message: "invalid_request")
        end
        if body["token_type_hint"].to_s == "refresh_token" && OAuthProtocol.find_token_by_hint(config[:store], body["token"].to_s, "access_token", **token_options.slice(:prefix, :format_refresh_token))
          raise APIError.new("BAD_REQUEST", message: "invalid_request")
        end
        if (token = OAuthProtocol.find_token_by_hint(config[:store], body["token"].to_s, body["token_type_hint"], **token_options.slice(:prefix, :format_refresh_token))) && token["clientId"].to_s == client_id.to_s
