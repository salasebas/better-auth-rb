...
module BetterAuth
  module SQLMigration
    ...
    def generate(options, dialect:, generator:, migrations_path: DEFAULT_MIGRATIONS_PATH, timestamp: Time.now.utc.strftime("%Y%m%d%H%M%S"))
      dialect = normalize_dialect(dialect)
      FileUtils.mkdir_p(migrations_path)
      path = File.join(migrations_path, "#{timestamp}_create_better_auth_tables.sql")
      return path if File.exist?(path)

      File.write(path, render(options, dialect: dialect, generator: generator))
      path
    end
