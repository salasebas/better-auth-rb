...
export const APIMethod = ({
	...
	const { props, code_prefix, code_suffix } = parseCode(children);
	const httpMethod = method ?? "GET";
	const authPath = `/api/auth${path}`;

	const httpCode = createHttpCurl({
		path: authPath,
		method: httpMethod,
		props,
	...
	const rubyCode = createRubyRackExample({
		path: authPath,
		method: httpMethod,
		props,
		requireSession: requireSession ?? false,
	...
	const endpointBlock = (
		<Endpoint method={httpMethod} path={path} className="" />
	);

	const notesBlock =
		serverOnlyNote || clientOnlyNote || note ? (
			<Note>
	...
	if (isExternalOnly) {
		return (
			<div className="border shadow-sm [&_figure]:my-0 [&_figure]:border-0 [&_figure]:shadow-none [&_figure]:rounded-none [&_.fd-scroll-container]:bg-transparent">
				{endpointBlock}
				{notesBlock}
				<DynamicCodeBlock code={httpCode} lang="bash" allowCopy />
				{parameterTable}
			</div>
		);
	}
	...
	return (
		<>
			...
			<ApiMethodTabs defaultValue="http" className="gap-0 w-full">
				...
				<ApiMethodTabsContent value="http">
					<div className="border shadow-sm [&_figure]:my-0 [&_figure]:border-0 [&_figure]:shadow-none [&_figure]:rounded-none [&_.fd-scroll-container]:bg-transparent">
						{endpointBlock}
						{notesBlock}
						{isClientOnly ? (
							<Note>
								This endpoint is typically invoked from a browser context
								(WebAuthn, redirects). Use HTTP for integration testing or
								server-side proxies.
							</Note>
						) : null}
						<DynamicCodeBlock code={httpCode} lang="bash" allowCopy />
				...
				<ApiMethodTabsContent value="ruby">
					<div className="border shadow-sm [&_figure]:my-0 [&_figure]:border-0 [&_figure]:shadow-none [&_figure]:rounded-none [&_.fd-scroll-container]:bg-transparent">
						{endpointBlock}
						{notesBlock}
						<DynamicCodeBlock code={rubyCode} lang="ruby" allowCopy />
						{parameterTable}
					</div>
				</ApiMethodTabsContent>
			</ApiMethodTabs>
...
function exampleValueForJson(prop: Property): string {
	if (prop.exampleValue) return prop.exampleValue.replace(/,$/, "");
...
function exampleValueForRuby(prop: Property): string {
	if (prop.exampleValue) {
		const value = prop.exampleValue.replace(/,$/, "");
...
function requestProps(props: Property[]): Property[] {
	return props.filter(
		(prop) => !prop.isServerOnly && prop.type !== "Object" && prop.path.length === 0,
	);
}

function buildJsonBody(props: Property[]): string {
	const relevant = requestProps(props);
	if (!relevant.length) return "{}";
	const lines = relevant.map(
		(prop) => `  "${prop.propName}": ${exampleValueForJson(prop)}`,
	);
	return `{\n${lines.join(",\n")}\n}`;
}

function buildRubyHash(props: Property[]): string {
	const relevant = requestProps(props);
	if (!relevant.length) return "{}";
	const lines = relevant.map(
		(prop) => `    ${prop.propName}: ${exampleValueForRuby(prop)}`,
	);
	return `{\n${lines.join(",\n")}\n  }`;
}

function buildQueryString(props: Property[]): string {
	const relevant = requestProps(props);
	if (!relevant.length) return "";
	return relevant
		.map((prop) => {
			const raw = exampleValueForJson(prop).replace(/^"|"$/g, "");
			return `${encodeURIComponent(prop.propName)}=${encodeURIComponent(raw)}`;
		})
		.join("&");
}

function createHttpCurl({
	path,
	method,
	props,
	requireSession,
	...
	const useQuery = shouldServerUseQueryParams(
		method,
		forceAsBody,
		forceAsQuery,
		forceAsParam,
	);
	const query = buildQueryString(props);
	const url = useQuery && query ? `${path}?${query}` : path;
	const headerLines = ['  -H "Content-Type: application/json"'];
	if (requireSession || requireHeaders) {
		headerLines.push('  -H "Cookie: better_auth.session=..."');
	}
	if (requireBearerToken) {
		headerLines.push('  -H "Authorization: Bearer <token>"');
	}

	let command = `${codePrefix}curl -X ${method} "$BASE_URL${url}" \\\n${headerLines.join(" \\\n")}`;

	if (!useQuery && requestProps(props).length > 0) {
		command += ` \\\n  -d '${buildJsonBody(props).replace(/'/g, "'\\''")}'`;
	}

	return `${command}${codeSuffix}`;
}

function createRubyRackExample({
	path,
	method,
	props,
	requireSession,
	...
	const headers: string[] = [`"CONTENT_TYPE" => "application/json"`];
	if (requireSession || requireHeaders) {
	...
	let body = `${codePrefix}# Rack::Test / integration style\n${verb} "${path}"`;
	const hash = buildRubyHash(props);
	if (useQuery && requestProps(props).length > 0) {
		body += `,\n  params: ${hash}`;
	} else if (requestProps(props).length > 0) {
		body += `,\n  params: ${hash}.to_json`;
	}
	if (headers.length) {
