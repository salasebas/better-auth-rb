...
export function LineFieldBackground({
	...
	return (
		<div
			className={
				variant === "panel"
					? "hidden lg:block absolute inset-0 overflow-hidden bg-transparent pointer-events-none"
					: "hidden lg:block absolute inset-0 overflow-hidden bg-background pointer-events-none"
			}
			aria-hidden="true"
		>
			<canvas
				...
				className={
					variant === "panel"
						? "w-full h-full invert opacity-[0.35] dark:invert dark:opacity-[0.28]"
						: "w-full h-full invert opacity-60 dark:invert-0 dark:opacity-70"
				}
			/>
		</div>
	);
