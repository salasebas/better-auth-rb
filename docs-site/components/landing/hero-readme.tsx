...
import { getLlmsTxtUrl } from "@/lib/branding";
...
const cliCommands = `gem install better_auth
# or Gemfile: gem "better_auth-rails"
bundle exec better-auth init --framework rails --cwd .
bundle exec better-auth generate --cwd . --dialect postgres
bundle exec better-auth migrate --cwd .`;

const aiPromptText = `Set up server-side authentication in my Ruby project using the better_auth gem (RubyAuth).

1. Add gem "better_auth" or "better_auth-rails" to the Gemfile and bundle install.

2. Configure BetterAuth with base_url, secret (32+ chars), and a database adapter
   (postgres, mysql, sqlite, mssql, or :memory for dev).

3. Mount the Rack app at /api/auth (Rails: routes.rb mount; Sinatra/Roda: plugin/extension).

4. Enable email_and_password and any social providers I have OAuth env vars for.

5. Run \`bundle exec better-auth generate\` and \`migrate\` for schema.

Read the full docs index at: ${llmsTxtUrl}`;
...
function InstallBlock() {
	const [mode, setMode] = useState<"cli" | "prompt">("cli");
			...
			>
				<div ref={contentRef}>
					<AnimatePresence mode="wait" initial={false}>
						<div>
							{mode === "cli" ? (
								<div className="flex items-start justify-between gap-3 bg-neutral-100/50 dark:bg-[var(--rubygems-slate)] px-4 py-3">
									<pre
										className="text-[11px] sm:text-[12px] text-neutral-700 dark:text-neutral-300 whitespace-pre-wrap leading-relaxed flex-1"
										style={{ fontFamily: "var(--font-geist-pixel-square)" }}
									>
										{cliCommands}
									</pre>
									<button
										onClick={() => copy(cliCommands)}
										className="text-neutral-400 dark:text-neutral-500 hover:text-neutral-600 dark:hover:text-neutral-300 transition-colors p-1 shrink-0"
								...
								<div className="bg-neutral-100/50 dark:bg-[var(--rubygems-slate)] px-5 py-4">
									<p className="text-[13px] font-medium text-neutral-700 dark:text-neutral-200 leading-relaxed">
										Set up server-side auth in my Ruby project using RubyAuth.
									</p>
									<div className="relative mt-1.5">
										<p className="text-[11px] text-neutral-400 dark:text-neutral-500 leading-relaxed line-clamp-2">
											Add the better_auth gem, configure BetterAuth with a database
											adapter, mount the Rack app, and run CLI migrations. Docs
											index included for your assistant...
										</p>
										<div className="absolute bottom-0 left-0 right-0 h-6 bg-gradient-to-t from-neutral-100/50 dark:from-[var(--rubygems-slate)] to-transparent pointer-events-none" />
									</div>
				...
				{promptOpen && (
					...
					>
						...
						<motion.div
							...
							className="relative w-[calc(100%-2rem)] max-w-lg mx-4 bg-neutral-50 dark:bg-[var(--rubygems-slate-light)] border border-neutral-200 dark:border-white/[0.06] rounded-sm shadow-2xl"
						>
							{/* Close */}
							<button
								onClick={() => setPromptOpen(false)}
		...
		<div className="mt-10 pt-8">
			<div className="flex items-center gap-4 mb-2">
				<a
					...
					className="text-lg font-medium text-foreground/90 dark:text-foreground/80 tracking-tight shrink-0 hover:text-foreground transition-colors"
				>
					Contributors
				</a>
			...
			<p className="text-[13px] text-foreground/50 dark:text-foreground/40 mb-5 leading-relaxed">
				...
				<a
					...
					target="_blank"
					rel="noopener noreferrer"
					className="text-foreground/70 dark:text-foreground/60 font-medium tabular-nums hover:text-foreground transition-colors"
				>
					{contributorCount}+
				</a>{" "}
				contributors.
					...
					>
						<div className="flex items-center gap-1.5 px-2.5 hover:bg-foreground/4 rounded-sm transition-colors text-foreground/50 dark:text-foreground/50">
							...
							<span className="text-xs font-mono">
								{formatCount(stats.rubygemsDownloads)} downloads
							</span>
						</div>
					</a>
					<a
							...
							{[
								...
								>
									...
									>
										...
										{"enterprise" in feature && feature.enterprise === true ? (
											<div className="mt-3 flex items-center gap-2.5">
												<div className="relative flex items-center gap-2.5">
													<div className="absolute left-3 right-3 top-1/2 h-px -translate-y-1/2 bg-foreground/[0.08]" />
													{/* Okta */}
