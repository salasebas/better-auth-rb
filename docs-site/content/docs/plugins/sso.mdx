...
<Steps>
  <Step>
    ...
    # Required to register and authenticate SAML providers.
...
<Callout type="info">
  For SAML providers, install `better_auth-saml` alongside `better_auth-sso`. When
  the gem is available, `BetterAuth::Plugins.sso` automatically configures
  `ruby-saml` to create AuthnRequests and verify SAML responses. No manual parser
  merge is required, and the SSO flow does not accept JSON/base64 assertions as a
  fallback.
</Callout>
...
When `organizationId` is provided and the organization plugin is enabled, the
caller must be an organization owner or admin. Provider IDs occupy an SSO
namespace for account linking: configure trusted SSO providers as
`sso:<providerId>` (for example, `sso:acme`) so a same-named social provider
cannot confer trust on an SSO connection.

Trusted provider or verified-domain status is not enough to attach a new OIDC
or SAML identity to an existing email-matched user: the local email must also be
verified by default. The deprecated
`account.account_linking.require_local_email_verified: false` option restores
the legacy implicit-linking behavior, but it is a high-risk compatibility escape
hatch and is not recommended. It does not affect authenticated, explicit account
linking.
...
SAML response verification is performed by `ruby-saml` through `better_auth-saml`
before an assertion can create or link a user. Applications that configure a custom
parser are responsible for validating the original SAML response before returning
its attributes.
...
Domain verification is optional and is only registered when `domain_verification.enabled` is true. When enabled, registering an SSO provider creates a `domainVerificationToken`, and the owner or organization admin can request the active token again or verify the domain after publishing the DNS TXT record. For a comma-separated domain list, publish the proof under every listed domain; verification succeeds only when every domain passes. Each TXT value may be either the raw token or `<identifier>=<token>`.
...
Deleting an SSO provider also deletes account links for that provider inside the
same database transaction. Users and identities from other providers are
preserved. Once account links exist, identity-defining settings such as issuer,
OIDC client/endpoints, SAML audience/endpoints, and subject mappings cannot be
changed; rotate secrets or certificates instead, or delete and recreate the
connection so stale identities cannot be reused.
...
If you want to allow account linking for specific trusted providers, enable the `accountLinking` option in your auth config and specify SSO providers in the `trustedProviders` list with the `sso:` prefix (for example, `sso:acme`). Bare provider names refer to the social-provider namespace and do not trust same-named SSO providers.

Implicit OIDC and SAML linking also requires the existing local user's email to
be verified. `requireLocalEmailVerified: false` is a deprecated, high-risk
compatibility escape hatch for legacy implicit linking and is not recommended;
authenticated explicit linking is unchanged.
