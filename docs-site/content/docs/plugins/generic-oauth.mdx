...
* **Yandex**: No additional required fields. Defaults to `login:info`, `login:email`, and `login:avatar`; it maps the Yandex profile and uses the `islands-200` avatar when available.

```ruby title="config/auth.rb"
BetterAuth::Plugins.generic_oauth(
  config: [
    BetterAuth::Plugins.yandex(
      client_id: ENV.fetch("YANDEX_CLIENT_ID"),
      client_secret: ENV.fetch("YANDEX_CLIENT_SECRET")
    )
  ]
)
```
...
* `access_token_expires_in?: number` - Fallback access-token lifetime in seconds
...
**accessTokenExpiresIn**: (Optional; Ruby key `access_token_expires_in`) Fallback access-token lifetime in seconds. It is used only when the provider token result omits an explicit expiry, including standard exchange, custom `get_token`, and refresh results. Provider-returned expiry always wins. Leave it unset when the provider returns `expires_in`.
