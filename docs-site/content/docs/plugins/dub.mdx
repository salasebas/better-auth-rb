...
auth = BetterAuth.auth(
  ...
  plugins: [
    BetterAuth::Plugins.dub(
      api_key: ENV.fetch("DUB_API_KEY"),
      oauth: {
        client_id: ENV.fetch("DUB_CLIENT_ID"),
        client_secret: ENV.fetch("DUB_CLIENT_SECRET")
      }
    )
  ]
)
...
For server-rendered Ruby apps, keep Dub's `dub_id` value in a cookie before sign-up so the plugin can attribute the lead when the user is created.

```ruby title="server.rb"
response.set_cookie("dub_id", {
  value: request.params["dub_id"],
  path: "/",
  same_site: "Lax"
})
```
