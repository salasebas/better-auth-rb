...
When using a custom `jwksPath` on the server, clients and services that fetch JWKS must use the same path. In Ruby, call `auth.api.get_jwks` or request that configured endpoint over HTTP:
