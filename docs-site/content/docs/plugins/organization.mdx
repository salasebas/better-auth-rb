...
To check if an organization slug is taken or not you can use the `checkSlug` function available through the Ruby API or HTTP endpoint. The function takes an object with the following properties:
...
You can customize organization operations using hooks that run before and after various organization-related activities. RubyAuth provides two ways to configure hooks:
...
To invite users to an organization, you can use the `invite` function available through the Ruby API or HTTP endpoint. The `invite` function takes an object with the following properties:
...
To get an invitation you can use the `organization.getInvitation` function available through the Ruby API or HTTP endpoint. You need to provide the invitation id as a query parameter.
