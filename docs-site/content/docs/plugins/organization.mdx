...
<Tabs items={["React", "Vue", "Svelte"]} default="React">
  <Tab value="React">
    ...
    organizations = auth.api.list_organizations(
...
<Tabs items={['React', 'Vue', 'Svelte']}>
  <Tab value="React">
    ```ruby title="server.rb"
    active = auth.api.get_full_organization(
      headers: {"cookie" => session_cookie}
    )
    ```
  </Tab>
...
Accept, reject, and get-by-ID always require the signed-in user's email to
match the invitation recipient. When invitation IDs use the default opaque ID
generator, matching is sufficient by default. Predictable or custom ID
generation also requires the recipient email to be verified. Set
`require_email_verification_on_invitation` explicitly to `true` or `false` to
override that ID-sensitive default.
...
<Callout type="info">
  Invitation acceptance transactionally claims the pending invitation, validates
  its role and capacity limits, and creates the organization and team memberships;
  those changes succeed or roll back together. Rejection only accepts a pending
  invitation owned by the recipient after applying any email-verification
  requirement, then updates its state.
</Callout>
...
The caller must be authenticated as the invitation recipient. Expired or
non-pending invitations are not returned.
...
To list invitations for the current user, use `listUserInvitations` through the
Ruby API or HTTP endpoint. The session email must be verified before invitations
targeted at that email can be enumerated.

```ruby title="server.rb"
invitations = auth.api.list_user_invitations(
  headers: {"cookie" => session_cookie}
)
```
...
The team must belong to the session's active organization and the user must be
a member of it. Selecting a team never changes `activeOrganizationId`; switch
organizations explicitly before selecting a team in another organization.
...
Every supplied team ID must belong to the invitation's organization. Team IDs
containing commas are rejected because invitations store multiple team IDs as a
comma-delimited value. If a team is removed while invitations are pending, that
team is removed from their assignments; invitations with no remaining teams
remain valid as organization-level invitations.
...
* `invitation_limit`: maximum number of pending invitations allowed across the organization. Defaults to `100`. Accepts a number or a callable receiving a symbol-keyed `{user:, organization:, member:}` data hash and the `BetterAuth::Context`; the callable must return the numeric threshold, which is used without integer coercion.
