...
When `displayUsername` is provided without a `username`, it is used as the username only when it passes username validation; otherwise it remains display-only. An explicit empty `username` is invalid. On successful username sign-in, `callbackURL` is echoed in the response and returned as the `Location` header when supplied.
