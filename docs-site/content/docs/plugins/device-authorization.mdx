...
The upstream Better Auth docs include a CLI demo command for trying the device
authorization flow end-to-end.

<UnderDevelopment />

RubyAuth does not currently ship an equivalent `better-auth login` device-flow
demo command. Use the HTTP/Ruby examples below to exercise the flow against your
own RubyAuth server.
...
<Callout type="info">
  The upstream CLI login command is a demo feature that connects to the Better Auth demo server to showcase the device authorization flow in action. It is not a RubyAuth CLI command.
</Callout>
...
<APIMethod path="/device/code" method="POST">
  ...
  type deviceCode = {
      ...
      /**
       * Optional user ID to pre-bind server-side.
       */
...
puts "Visit #{issued[:verification_uri]}"
puts "Enter code: #{issued[:user_code]}"
```
...
begin
  ...
  raise unless error.code == "authorization_pending"
end
```
...
2. **Authorization**: The same authenticated user approves or denies the device
...
class DeviceVerificationPage
  ...
  def call(env)
    ...
    verification = @auth.api.device_verify(
      headers: {"cookie" => request.env.fetch("HTTP_COOKIE", "")},
      query: {user_code: user_code}
    )

    [200, {"content-type" => "text/plain"}, [
      "Device request is #{verification[:status]}"
    ]]
  rescue BetterAuth::APIError
    [400, {"content-type" => "text/plain"}, ["Invalid or expired code"]]
  end
end
```
...
An authenticated `GET /device` atomically claims an unbound pending code for
that user. Approval and denial require this claim and reject a different user,
even if that user knows the code. Render the verification page with the user's
session cookie before submitting either decision. A trusted server may instead
pre-bind the code by sending `user_id` when requesting it; an empty `user_id` is
treated as omitted.
...
user_code = request.params.fetch("user_code")

if request.post? && request.params["decision"] == "approve"
  auth.api.device_approve(
    headers: {"cookie" => cookie},
    body: {userCode: user_code}
  )
else
  auth.api.device_deny(
    headers: {"cookie" => cookie},
    body: {userCode: user_code}
  )
end
  ...
  plugins: [
    BetterAuth::Plugins.device_authorization(
      ...
      validate_client: ->(client_id) {
        %w[demo-cli tv-app].include?(client_id)
      }
    )
  ]
)
...
auth = BetterAuth.auth(
  ...
  plugins: [
    BetterAuth::Plugins.device_authorization(
      generate_device_code: -> { SecureRandom.alphanumeric(48) },
