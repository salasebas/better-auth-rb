...
<Callout type="info">
  When a sign-in OTP proves control of an email for an existing unverified user,
  RubyAuth removes every pre-proof credential account and active session before
  marking the email verified. It then creates one new session for the mailbox
  owner. This does not change the separate email-verification OTP flow.
</Callout>
