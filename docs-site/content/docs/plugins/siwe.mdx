...
For compatibility with upstream Better Auth clients, the plugin also exposes
`POST /api/auth/siwe/get-nonce`. Its direct Ruby API accepts the upstream
`address` field as an alias for `walletAddress`:

```ruby title="server.rb"
nonce = auth.api.get_nonce(
  body: {
...
Both nonce endpoints accept either address field and use the same validation,
15-minute storage lifetime, and `{ nonce: ... }` response.
