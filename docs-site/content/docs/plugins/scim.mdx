...
If you're using [RubyAuth Infrastructure](https://dash.better-auth.com/sign-in), you get self-service directory sync in the dashboard. Organization admins can create and manage SCIM directory connections and rotate bearer tokens without calling the SCIM APIs directly.
...
A `SCIM` token is always restricted to a provider, thus you are required to specify a unique `providerId`. It cannot collide with the six fixed account provider IDs (`credential`, `email-otp`, `magic-link`, `phone-number`, `anonymous`, and `siwe`), a configured social provider (including a disabled entry), a resolved provider, a default SSO provider, or a persisted SSO provider.
...
<Callout>
  **Important:** Personal SCIM connections can still be generated by any authenticated user unless `can_generate_token` restricts them. Organization-scoped connections are restricted by default to users with the `admin` role or the organization creator role (`organization.creatorRole`, which defaults to `owner`). The callback runs after these built-in checks and cannot bypass them.
</Callout>

#### Organization-scoped authorization

When `organizationId` is provided, RubyAuth requires the current user to be a member of that organization and to have at least one of the configured `requiredRole` values.
...
Delete an existing connection. This immediately invalidates its token but does
not deprovision or delete accounts previously created through the connection.
...
Provisions a new user to the database. The user will have an account associated to the same provider and will be member of the same org than the SCIM token. Existing email matches fail with SCIM `409` unless `link_existing_users` explicitly allows linking.
...
Replaces an existing user details in the database. This operation can only update users that belong to the same provider and organization than the SCIM token. Email changes are lowercased, uniqueness checked, and reset `emailVerified`; case-only writes preserve verification. Writing `active: false` requires the admin plugin and revokes all sessions.
...
Deprovisions a user within the token's authority boundary. Organization-scoped tokens preserve the global user while removing the organization member, organization team memberships, and current SCIM account. Personal tokens unlink the current provider account and delete the global user only when no other identity remains.
...
* `provider_ownership`: `{ enabled: boolean }` — When enabled, links each personal provider connection to the user who generated its token. See [Connection ownership](#scim-provider-connection-ownership) for details. The Ruby port defaults to `{ enabled: true }`.
...
* `default_scim`: Default list of SCIM tokens for testing.
* `store_scim_token`: The method to store the SCIM token in your database, whether `encrypted`, `hashed` or `plain` text. The Ruby port defaults to `hashed`.
* `link_existing_users`: Disabled by default. `true` restores unrestricted legacy email linking. Prefer a policy hash with `trusted_domains`, `require_existing_org_membership`, and/or `should_link_user`; every supplied constraint must pass and an empty hash denies linking.
* `can_generate_token`: Callback receiving `user`, `provider_id`, optional `organization_id`, and `member` (`nil` for personal tokens). Return false to deny issuance after built-in checks.
...
BetterAuth::Plugins.scim(
  ...
  can_generate_token: ->(payload) { payload[:organization_id] != nil }
)
```

When migrating from legacy email linking, do not enable `true` by default. Use
positive constraints unless all SCIM token issuers are trusted to take control
of existing matching identities. The admin plugin is required to accept
`active: false`; deactivation uses the admin banned state and revokes sessions.
