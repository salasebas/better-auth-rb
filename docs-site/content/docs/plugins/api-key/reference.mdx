...
Defer pure secondary-storage counter writes, expired or exhausted key deletion, and successful-verification cleanup using the global `backgroundTasks` handler. Database-backed verification counter claims remain synchronous. Default is `false`. In Ruby configuration hashes, use `defer_updates`.

Requires `backgroundTasks.handler` to be configured in the main auth options.

<Callout type="warn">
  In pure secondary-storage mode, the response returns optimistic counter data before the secondary record is updated. Only enable if your application can tolerate this eventual consistency.
</Callout>
