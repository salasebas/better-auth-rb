...
* Verification accepts a cache hit without re-reading the database and queries
  the database only on a cache miss. Counter claims write through the database,
  then refresh the cache from the resulting row.
...
An exhausted non-refillable key is deleted during verification and rejected
with `USAGE_EXCEEDED`. With `defer_updates`, deletion runs through the configured
