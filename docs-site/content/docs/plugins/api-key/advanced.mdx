...
* The database is authoritative for verification. Cache refresh follows a
  successful guarded database write, but the cross-store window before that
  cache write can leave a stale entry or briefly re-create one. Verification
  always re-reads the database; a missing authoritative row invalidates the
  cache and can never authorize.
* Generic custom storage serializes reference-list updates within one Ruby
  process only. Redis-backed storage uses atomic JSON-list operations for
...
* When verifying an API key with `auth.api.verify_api_key`
...
Rate-limit rejections use HTTP status `429` and error code `RATE_LIMITED`.
Authentication failures (missing, disabled, or expired keys) continue to use
HTTP `401`.

Database-backed counters use guarded atomic updates, so concurrent requests
cannot exceed `rateLimitMax` or drive `remaining` below zero. Pure
`secondary-storage` mode has no guarded JSON-record counter primitive and is
best-effort across processes; use `fallback_to_database: true` for strict
enforcement.
...
Whenever an API key is used, the `remaining` count is updated. If the `remaining` count is `null`, then there is no cap to key usage. Otherwise, the `remaining` count is decremented by 1. Guarded database updates prevent it from becoming negative; an exhausted key is rejected with HTTP `429`.

Ruby keeps an exhausted non-refillable row inert during verification instead of
eagerly deleting it: a cross-process atomic delete-plus-counter operation is
not available. Cleanup routes or jobs may remove exhausted rows separately.
