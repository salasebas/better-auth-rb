...
The **Captcha Plugin** integrates bot protection into your RubyAuth system by adding captcha verification for key endpoints. This plugin ensures that only human users can perform actions like signing up, signing in, or resetting passwords. The following providers are currently supported:
...
<Steps>
  <Step>
    ...
    auth = BetterAuth.auth(
      ...
      plugins: [
        BetterAuth::Plugins.captcha(provider: "cloudflare-turnstile", secret_key: ENV.fetch("TURNSTILE_SECRET_KEY"))
      ]
    )
    ```
  </Step>
...
* `expectedAction` (optional - Google reCAPTCHA and Cloudflare Turnstile): rejects tokens minted for a different action.
* `allowedHostnames` (optional - Google reCAPTCHA and Cloudflare Turnstile): accepts tokens only from the configured hostnames.

Provider verification is bounded to 10 seconds and fails closed. `/sign-in/email-otp` is excluded from the default protected endpoints; add it explicitly to `endpoints` to protect it.
