...
* `customStorage`: A custom backend. Implement atomic `consume(key, window:, max:) => {allowed:, retry_after:}` for strict concurrency enforcement. Legacy `get`/`set` backends remain best effort and emit one warning.

Memory and database storage use rolling windows. Memory is scoped to one auth instance and capped at 100,000 keys. Secondary storage with `increment` uses a fixed first-request window and returns the full window as retry-after when blocked. Rate limiting runs before plugin `on_request` but after plugin router middleware; plugin `on_response` still receives `429` responses.
...
* `ip_address`: IP address configuration for rate limiting and session tracking
  * `disable_ip_tracking`: Disable IP tracking and the built-in per-IP limiter.
  * `ip_address_headers`: Explicitly trusted, ingress-controlled header names, checked in order.
  * `trusted_proxies`: Proxy IP addresses or CIDR ranges used to resolve configured forwarded chains from right to left.
  * `ipv6_subnet`: IPv6 prefix length used to group rate-limit keys (default: `64`).
...
* `default_find_many_limit`: The maximum number of records returned when a core Memory or SQL adapter `find_many` call omits `limit:` or passes `nil`. The same default applies to one-to-many joins without their own limit. An explicit `limit:` overrides it. Use non-negative values for portable behavior; `0` returns no base rows, while negative pagination is adapter-specific and does not mirror JavaScript `slice` semantics. (default: `100`)
