...
During the OAuth callback, RubyAuth expects a `state` value to be present on the incoming request.
...
### Start the flow via RubyAuth APIs

Always initiate OAuth through RubyAuth so we can generate and send `state` correctly. Avoid manually hitting
callback endpoints or constructing authorize URLs unless you fully mirror RubyAuth's parameters.
