...
When an OAuth or SSO flow begins, RubyAuth generates a unique `state` value and stores it so it can be
...
RubyAuth supports two state storage strategies - **database** (the default) and **cookie** - and each
...
| `state_mismatch`          | request expired                       | Both     | The state data was found but its `expires_at` timestamp is in the past.                        |
...
3. **Secondary storage (Redis / KV) without database fallback.** When `secondary_storage` is configured,
...
* If using `secondary_storage`, either set `verification.storeInDatabase: true` as a fallback, or ensure
...
`expires_at` timestamp has passed. The state payload is valid for 10 minutes from creation.
...
After the verification record is found in the database, RubyAuth also checks that a **signed state
...
* For SAML flows, RubyAuth already sets `skipStateCookieCheck` internally.
...
* Run `bundle exec better-auth migrate --cwd . --config config/better_auth.rb --yes` to ensure all tables exist.
* Check your database connection and credentials.
* Review any `database_hooks` on the `verification` model that might prevent writes.
...
| **Low**       | Missing verification table / migration not run                        | `state_generation_error`                                        | Run `bundle exec better-auth migrate --cwd . --config config/better_auth.rb --yes`     |
