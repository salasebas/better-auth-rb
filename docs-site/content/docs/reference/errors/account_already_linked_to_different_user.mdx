...
another user in your project. To prevent account takeover, RubyAuth blocks the link and
throws this error.
...
* You rely on email matching to decide linking, but the actual unique key is the provider account identifier
  (e.g., `provider_id` + `accountId`). If that mapping exists for another user, linking will be blocked.
...
* Inspect your `account` database table. You should see rows keyed by
  `provider_id` (e.g., 'google') and `accountId` (e.g., OIDC `sub`), pointing to a `user_id`.
