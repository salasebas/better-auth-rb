...
`code` parameter (and typically `state`). Without the `code`, RubyAuth cannot exchange it for tokens,
...
* Start the flow through RubyAuth so the provider receives the correct parameters and the app expects a `code`.
