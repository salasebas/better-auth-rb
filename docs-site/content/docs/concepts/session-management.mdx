...
Because this exposes authoritative session state, it requires a fresh database-backed session. A stale cached session receives `403 SESSION_NOT_FRESH`.
...
If you don't pass a database or `secondary_storage` configuration, RubyAuth will automatically enable stateless mode.
...
<Callout type="info">
  If you don't provide a database or secondary storage, we provide the above configuration by default.
</Callout>
...
* **`false`** (default for stateful/server-store configurations): No automatic refresh. When the cookie cache expires (reaches `max_age`), it will attempt to fetch from the database if available. Fully stateless configurations, without a database or secondary storage, default to `true`.
* **`true`**: Enable automatic refresh with default settings. Refreshes after 80% of `max_age` has elapsed, when less than 20% remains.
...
auth = BetterAuth.auth(
  session: {
    cookie_cache: {
      ...
      refresh_cache: {
        update_age: 60 # Refresh when fewer than 60 seconds remain before expiry
      }
    }
  }
)
