...
* **`false`** (default for stateful/server-store configurations): No automatic refresh. When the cookie cache expires (reaches `max_age`), it will attempt to fetch from the database if available. Fully stateless, database-less configurations default to `true`.
